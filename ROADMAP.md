# ntccpay — NT Credit Card Payment Authorization Platform
...
- **Java 25 (LTS)** + **Spring Boot 4.x** (Web, **Security**, Data JPA, Kafka, Actuator, Validation)
- **PostgreSQL** (ledger, settlement) + **Redis** (velocity counters, hot BIN cache)
...
- [ ] Install **JDK 25 (LTS)** (Eclipse Temurin — via winget or IntelliJ's JDK downloader)
- [ ] Install **Docker Desktop** (or Podman); verify `docker run hello-world`
- [ ] IntelliJ: Lombok plugin enabled, Project SDK = 21
- [ ] GitHub repo pushed; `.gitignore` for Maven/Gradle + IDE
- [ ] Skim: what a Maven/Gradle build is, what a Spring Boot "fat jar" is

### Phase 1 — The auth decision, in-memory (Week 1–2)
> Learn: Spring Boot fundamentals, hexagonal architecture, TDD, REST, idempotency.

- [ ] Generate project on Spring Initializr → **auth-api**: Web, **Security**, Validation, Actuator, Lombok
...
- [ ] **AuthN with Spring Security**: per-merchant API key (`X-API-Key`) via a custom filter; stateless (no sessions, CSRF disabled); unauthenticated → 401 problem+json (not the default login-page redirect); write the ADR for API-key vs OAuth2 client credentials
...
- [ ] Spring Data JPA + Flyway; Postgres via **Testcontainers** in tests
...
- [ ] **ledger-service**: consumes `auths.v1`, writes double-entry rows (debit issuer-pending, credit merchant receivable); idempotent via `processed_events` table keyed by event id
...
- [ ] Define `AuthorizationAuthorized` as a versioned JSON event schema, documented in the repo
...
> Learn: service decomposition, REST clients, resilience, Redis.
...
- [ ] **Client-side retry**: Resilience4j Retry with exponential backoff + **jitter** on the decision-service call; retries are safe *because* the request carries an idempotency key — note this pairing in the ADR (retry without idempotency = duplicate payments)
- [ ] **Graceful degradation**: when the circuit breaker is open, fall back to the stale Caffeine BIN cache and mark decisions `DEGRADED`; shed load with `503 + Retry-After` when saturated — a broken dependency slows you down, it must not take you down
- [ ] **Rate limiting**: bucket4j + Redis per-merchant limiter on the auth endpoint (resolves the open gap in `docs/security.md`) — a misbehaving merchant cannot exhaust the auth path
- [ ] **fraud-service**: consumes `auths.v1`, computes velocity/risk scores asynchronously, publishes `fraud.flags.v1`
...
latency budget with decision-service behind a circuit breaker (retries with
backoff, `DEGRADED` fallback when it's down); the settlement file
...
- [ ] Define SLOs: 99.9% of auths < 150ms; **availability SLO + error budget** alongside it (e.g. 99.9% of auth requests succeed over 30 days; the budget decides when feature work stops and reliability work starts) — alert rules in Grafana
- [ ] Load test with **k6** or **Gatling**: ramp to realistic TPS; find the bottleneck (DB pool? rule engine? serialization?)
- [ ] JVM tuning: enable **virtual threads** (`spring.threads.virtual.enabled=true`), capture/read a JFR recording, GC choice, heap sizing
...
- [ ] **PodDisruptionBudgets** + rolling-update strategy (maxSurge/maxUnavailable); prove **zero-downtime deploy**: run k6 load, deploy a new version mid-run, zero failed auths
...
- [ ] Point Helm releases at EKS; images pulled from ECR
...
6. **Mind the PAN.** Treat card data as toxic: mask in logs (`****1111`), never commit fixtures with real card numbers, use test BINs only (e.g., Stripe's `4242...`).
...
2. Go to [start.spring.io](https://start.spring.io): Maven, Java 25, Boot 4.x, artifact `auth-api`;
   dependencies: Spring Web, Security, Validation, Actuator, Lombok. Add Cucumber via
